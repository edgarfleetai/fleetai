...
def clean_desc(text, car_code, words, amount):
    desc = text
    if car_code:
        desc = re.sub(r"^\s*" + re.escape(str(car_code)) + r"\b", "", desc).strip()
    for word in sorted(words, key=len, reverse=True):
        desc = re.sub(r"\b" + re.escape(word) + r"\b", "", desc).strip()
    if amount:
...
def parse_message(message):
    ...
    data = {
        "raw": raw, "car_code": None, "type": "unknown", "category": "",
        "description": "", "part": "", "brand": "", "position": "",
        "part_price": 0, "labor": 0, "total": 0, "income": 0,
        "mileage": None, "share_type": "shared", "investor_name": "",
        "investor_percent": 0, "days": 0, "start_date": None,
        "end_date": None, "active": 0,
    }
    ...
    if any(re.search(r"\b" + re.escape(w) + r"\b", text) for w in investment_words):
        ...
        data["description"] = clean_desc(text, data["car_code"], investment_words, data["total"]) or "Дополнительное вложение"
        return data
